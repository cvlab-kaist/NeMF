...
class SemanticKeypointsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        batch['src_img'] = src_numpy
        batch['trg_img'] = trg_numpy

        # Key-points (re-scaled)
        batch['src_kps'], num_pts = self.get_points(self.src_kps, idx, batch['src_imsize_ori'])  # Nx2
        batch['trg_kps'], _ = self.get_points(self.trg_kps, idx, batch['trg_imsize_ori'])  # Nx2
    ...
    def recover_image_pair_for_training(self, batch):
        ...
        source = batch['src_img']
        target = batch['trg_img']
        kp_source = batch['src_kps'].clone()
        kp_target = batch['trg_kps'].clone()
        n_pts = batch['n_pts']
    ...
    def horizontal_flip(self, batch):
        ...
        batch['src_bbox'][0] = batch['src_img'].shape[1] - batch['src_bbox'][2]
        batch['src_bbox'][2] = batch['src_img'].shape[1] - tmp

        tmp = batch['trg_bbox'][0].clone()
        batch['trg_bbox'][0] = batch['trg_img'].shape[1] - batch['trg_bbox'][2]
        batch['trg_bbox'][2] = batch['trg_img'].shape[1] - tmp

        batch['src_kps'][:batch['n_pts'], 0] = batch['src_image'].shape[1] - batch['src_kps'][:batch['n_pts'], 0]
        batch['trg_kps'][:batch['n_pts'], 0] = batch['trg_image'].shape[1] - batch['trg_kps'][:batch['n_pts'], 0]

        batch['src_image'] = np.flip(batch['src_image'], 1)
        batch['trg_image'] = np.flip(batch['trg_image'], 1)
