...
from models.mlp_optimization import MLPTrainer
...
class TransformerAggregator(nn.Module):
    ...
    def forward(self, corr, source, target):
        ...
        x = torch.cat((x.transpose(-1, -2), target.repeat_interleave(16, dim=1)), dim=3) + pos_embed.repeat_interleave(16, dim=1)
        # x = B, C, 256, 256
        x = self.proj(self.blocks(x)).transpose(-1, -2) + corr  # swapping the axis for swapping self-attention.
      
        x = torch.cat((x, source.repeat_interleave(16, dim=1)), dim=3) + pos_embed.repeat_interleave(16, dim=1)
...
class NeMF(nn.Module):
    ...
    mlp_image_size=512):
        ...
        self.corr_embedder = nn.ModuleList([CostEmbedding4d_64(), CostEmbedding4d_32(), CostEmbedding4d_16(), CostEmbedding4d_8()])
        ...
        self.mlp_model = MLPTrainer(enc_feature_size=feature_size, img_size=mlp_image_size)
    ...
    def forward_enc(self, target, source):
        B, _, H, W = target.size()
        ...
        for i, (src, tgt) in enumerate(zip(src_feats, tgt_feats)):
            ...
            corrs.append(corr)
            src = F.interpolate(src, self.feature_size, None, 'bilinear', True)
            tgt = F.interpolate(tgt, self.feature_size, None, 'bilinear', True)
        ...
        for i, c in enumerate(corrs):
            c = self.corr_embedder[i](c)
            corrs_embedded.append(c)
        
        # B, C, 16, 16, 16, 16
        corr = torch.cat(corrs_embedded, dim=1).view(-1, 64, 256, 256)
        corr = self.mutual_nn_filter(corr)
        
        refined_corr = self.decoder(corr, src_feats, tgt_feats)
        corr_enc = refined_corr.mean(1)
        grid_x, grid_y = self.soft_argmax(corr_enc.view(B, -1, self.feature_size, self.feature_size))
        flow = torch.cat((grid_x, grid_y), dim=1)
        flow_enc = unnormalise_and_convert_mapping_to_flow(flow)

        return refined_corr, flow_enc, corr, corr_enc

    def forward(self, target_enc, source_enc, target, source, trg_kps, src_kps):
        
        refined_corr, flow_enc, _, projected_corr = self.forward_enc(target_enc, source_enc)
        logits_list = self.mlp_model(refined_corr, projected_corr, source, target, trg_kps, src_kps)
        
        return flow_enc, logits_list

    def forward_inference(self, target_enc, source_enc, trg_kps, src_kps, trg_imsize, src_imsize):
        with torch.no_grad():
            self.feature_extraction.eval()
            refined_corr, flow_enc, _, projected_corr = self.forward_enc(target_enc, source_enc)
        
        flow = self.mlp_model.forward_patchmatch(refined_corr, projected_corr, src_kps, trg_kps, src_imsize, trg_imsize)
        
        return flow, flow_enc
