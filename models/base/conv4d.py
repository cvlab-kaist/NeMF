...
class Encoder_T_4D(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        self.conv4d = nn.ModuleList([])
        for i, (k, s, p, o, t) in enumerate(zip(kernel_size, stride, padding, out_padding, target_size)):
         
            conv4d = nn.Sequential(
                Conv4d(corr_levels[i], corr_levels[i + 1], k, s, p, True, True, True, t, o),
                nn.GroupNorm(group[i], corr_levels[i + 1]),
                nn.ReLU() # No inplace for residual
            )
            self.conv4d.append(conv4d)
