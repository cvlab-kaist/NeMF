...
class Evaluator:
    ...
    @classmethod
    def eval_kps_transfer(cls, prd_kps, batch, alpha=0.1):
        r"""Compute percentage of correct key-points (PCK) based on prediction"""
        thres = batch['pckthres'][0]
        src_kps = batch['src_kps'].squeeze(0).t()[:, :batch['n_pts'][0]].to(prd_kps.device)
        npt = src_kps.size(-1)
        _, correct_ids, _ = cls.classify_prd(prd_kps.squeeze(0), src_kps, thres, alpha)

        return (len(correct_ids) / npt) * 100

    @classmethod
    def eval_kps_transfer_eval(cls, prd_kps, batch, alpha):
        r"""Compute percentage of correct key-points (PCK) based on prediction"""
        ...
        pck = []
        for idx, (pk, tk) in enumerate(zip(prd_kps, batch['src_kps'].to(prd_kps.device))):
            thres = batch['pckthres'][idx]
            npt = batch['n_pts'][idx]
            _, correct_ids, _ = cls.classify_prd(pk[:, :npt], tk[:, :npt], thres, alpha)
    ...
    @classmethod
    def eval_kps_transfer_test(cls, prd_kps, batch, alpha):
        r"""Compute percentage of correct key-points (PCK) based on prediction"""
        ...
        pck = []
        src_kps = rearrange(batch['src_kps'], 'B C N -> B N C').to(prd_kps.device)
        for idx, (pk, tk) in enumerate(zip(prd_kps, src_kps)):
            thres = batch['pckthres'][idx]
            npt = batch['n_pts'][idx]
            _, correct_ids, _ = cls.classify_prd(pk[:, :npt], tk[:, :npt], thres, alpha)
    ...
    @classmethod
    def eval_kps_transfer_with_correct(cls, prd_kps, batch, alpha):
        r"""Compute percentage of correct key-points (PCK) based on prediction"""
    ...
    def classify_prd(cls, prd_kps, trg_kps, pckthres, alpha):
        ...
        thres = (pckthres.expand_as(l2dist).float() * alpha).to(prd_kps.device)
